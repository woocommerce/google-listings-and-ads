...
- Intermediate booleans that act as one-step relays (e.g. `$isInvalid = …; if ( $isInvalid )`) — consolidate unless the name genuinely adds clarity
- Variables not used before being assigned
- Strict comparisons (`===` / `!==`) — flag loose comparisons where strictness matters; note `=== false` when it's intentional vs. fragile
...
- Optimistic UI updates account for failure — flag cases where a failed API call could leave the UI in a broken state
...
- Intermediate booleans that are one-step relays without adding clarity — consolidate
- Strict comparisons — flag `=== false` where it may be fragile vs. intentional
- Hook names, utility names, and component names are descriptive but not redundant given domain context
- Array index as `key` prop — flag unless the list is static and never reordered

**SCSS**
- Reuse existing SCSS variables (`$`, from auto-imported abstracts) rather than hardcoding values
...
Each finding must be actionable and explain the why, not just the what. "This will break on empty arrays because X" is a comment; "this is wrong" is not.
