...
class PolicyComplianceCheckControllerTest extends RESTControllerUnitTest {
	...
	protected const ALLOWED_COUNTRIES_VERIFICATION     = '/wc/gla/mc/policy_check/allowed_countries';
	protected const STORE_SSL_VERIFICATION     = '/wc/gla/mc/policy_check/store_ssl';
	protected const PAYMENT_GATEWAYS_VERIFICATION     = '/wc/gla/mc/policy_check/payment_gateways';
	protected const REFUND_RETURN_VERIFICATION     = '/wc/gla/mc/policy_check/refund_return_policy';
	...
	public function test_allowed_countries_verification() {
		$this->policy_compliance_check->expects( $this->once() )
		                         ->method( 'get_allowed_countries' )
		                         ->willReturn([ 'allowed_countries' => ['US', 'UK'] ]);

		$response = $this->do_request( self::ALLOWED_COUNTRIES_VERIFICATION, 'POST', [] );

		$this->assertEquals( 200, $response->get_status() );
		$this->assertEquals( [ 'allowed_countries' => ['US', 'UK'] ], $response->get_data() );
	}

	public function test_store_ssl_verification() {
		$this->policy_compliance_check->expects( $this->once() )
		                         ->method( 'get_is_store_ssl' )
		                         ->willReturn( [ 'ssl' => true ] );

		$response = $this->do_request( self::STORE_SSL_VERIFICATION, 'POST', [] );

		$this->assertEquals( 200, $response->get_status() );
		$this->assertEquals( [ 'store_ssl' => true ], $response->get_data() );
	}

	public function test_payment_gateways_verification() {
		$this->policy_compliance_check->expects( $this->once() )
		                         ->method( 'has_payment_gateways' )
		                         ->willReturn( [ 'payment_gateways' => true ] );

		$response = $this->do_request( self::PAYMENT_GATEWAYS_VERIFICATION, 'POST', [] );

		$this->assertEquals( 200, $response->get_status() );
		$this->assertEquals( [ 'payment_gateways' => true ], $response->get_data() );
	}

	public function test_refund_return_policy_verification() {
		$this->policy_compliance_check->expects( $this->once() )
		                         ->method( 'has_refund_return_policy_page' )
		                         ->willReturn( [ 'refund_return_policy_page' => true ] );

		$response = $this->do_request( self::REFUND_RETURN_VERIFICATION, 'POST', [] );
