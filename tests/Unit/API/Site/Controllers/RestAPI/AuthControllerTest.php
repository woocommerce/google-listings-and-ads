...
class AuthControllerTest extends RESTControllerUnitTest {
	...
	public function test_authorize_with_referrer_params_appends_them_to_store_url() {
	...
	public function test_authorize_without_referrer_params_does_not_append_them() {
		...
		$this->oauth_service->expects( $this->once() )
			...
			->with(
				$this->callback(
					function ( $store_url ) {
						$this->assertStringNotContainsString( 'referrer_type', $store_url );
						$this->assertStringNotContainsString( 'referrer_id', $store_url );
						return true;
					}
				)
			)
