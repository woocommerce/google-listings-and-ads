...
class MarketServiceTest extends UnitTest {
	...
	public function test_delete_market_fires_woocommerce_gla_market_deleted_hook_once_with_id_and_country(): void {
		$this->set_up_options_get(
			[
				OptionsInterface::MARKETS => [
					'fr' => [
		...
		$captured = [];
		$listener = function ( $market_id, $country ) use ( &$captured ) {
			$captured[] = [ $market_id, $country ];
		};
		...
		$this->assertSame( [ 'fr', 'FR' ], $captured[0] );
	}
