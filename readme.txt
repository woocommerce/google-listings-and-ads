...
= 3.9.1 - 2026-08-24 =
* Add - Automatically fold a new market into the Primary market — in any shipping mode — when its shipping configuration already matches, instead of creating a duplicate feed, with a confirmation message naming the country.
* Dev - E2E tests for market modals.
...
* Fix - Keep a market with flat shipping intact instead of silently deleting it when its rate is edited to match the Primary market or when the store's shipping method is switched (existing per-country markets are migrated automatically on upgrade).
...
* Fix - Stop deleting a market on a multilingual store from silently re-adding its country to the Primary market; deletion is now final.
