...
class ProductHelper implements Service {
	...
	public function get_wc_product_id( string $mc_product_id ): int {
		/**
		 * Filters the Merchant Center product ID into a {slug}_{id} format before it's converted to
		 ...
		 * @param string $prefix        The slug prefixed to generated Merchant Center product IDs.
		 * @since x.x.x
		 *
		 * @return string Merchant Center product ID as normally generated by the plugin (e.g., gla_1234).
		 */
		$mc_product_id = apply_filters( 'woocommerce_gla_get_wc_product_id', $mc_product_id, $this->get_slug() . '_' );
