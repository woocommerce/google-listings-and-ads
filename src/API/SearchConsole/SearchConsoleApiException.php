...
/**
 * Class SearchConsoleApiException
 *
 * Wraps a non-2xx response from the Search Console Sites API.
 *
 * @package Automattic\WooCommerce\GoogleListingsAndAds\API\SearchConsole
 */
class SearchConsoleApiException extends Exception implements GoogleListingsAndAdsException {
	...
	/**
	 * SearchConsoleApiException constructor.
	 *
	 * @param int            $http_status   HTTP status code from the Sites API response.
	...
	public function __construct( int $http_status, array $response_body, string $method, ?Throwable $previous = null ) {
		...
		$message = $response_body['error']['message'] ?? 'Search Console API request failed';
