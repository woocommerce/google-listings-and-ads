	...
	protected function handle_actions() {
		...
		if ( 'wcs-google-sv-token' === $_GET['action'] && check_admin_referer( 'wcs-google-sv-token' ) ) {
			...
			do {
				...
				$this->response = "Attempting verification with $site_url\n\n";
				...
				try {
					...
					$this->response .= 'Error generating token: ';
					$this->response .= "\n" . print_r( $e->getMessage(), 1 );
					break;
				}

				$this->response .= 'Meta tag retrieved (Google API): ' . htmlentities( $meta_tag );

				/** @var Options $options */
				$options        = $this->container->get( OptionsInterface::class );
				$this->response .= "\nAdding " . OptionsInterface::SITE_VERIFICATION . " to wp_options ...";
				$options->update(
				...
				$this->response .= " DONE ==> " . htmlentities( json_encode( $options->get( OptionsInterface::SITE_VERIFICATION ) ) );

				$this->response .= "\n\nVerifying site...";

				// Manual verification SIMULATION by retrieving HTML (works for local sites)
				if ( 1 ) {
					...
					if ( $found ) {
						...
						$this->response .= "\nUNABLE TO VERIFY (manual wp_remote_get)!";
					}
				}

				// Google API verification (only works for live sites)
				if ( 1 ) {
					try {
						if ( $site_verification->insert( $site_url ) ) {
							$verification_options['verified'] = 'yes';
							$options->update( OptionsInterface::SITE_VERIFICATION, $verification_options );
							$this->response .= "\nMeta tag found (using Google API) and verified!";
						}
					} catch ( Exception $e ) {
						$this->response .= "\nError verifying site (Google API): ";
						$this->response .= "\n" . print_r( $e->getMessage(), 1 );
					}
				}
